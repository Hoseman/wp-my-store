<?php
/**
 * My Store functions and definitions
 *
...
if ( ! function_exists( 'my_store_setup' ) ) :
	/**
	 ...
	 */
	function my_store_setup() {
		/*
		 * Make theme available for translation.
		 * Translations can be filed in the /languages/ directory.
		 * If you're building a theme based on My Store, use a find and replace
		 * to change 'my-store' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'my-store', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'my-store' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'my_store_custom_background_args',
				array(
...
add_action( 'after_setup_theme', 'my_store_setup' );
 ...
 */
function my_store_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'my_store_content_width', 640 );
}
add_action( 'after_setup_theme', 'my_store_content_width', 0 );
...
function my_store_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'my-store' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'my-store' ),
 ...
 */
function my_store_scripts() {
	wp_enqueue_style( 'my-store-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'my-store-style', 'rtl', 'replace' );

	wp_enqueue_script( 'my-store-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'my_store_scripts' );
